...
func (c *Client) Login(ctx context.Context, id, password string) (*Client, error) {
	u, err := urlpkg.Parse(c.endpoint)
	if err != nil {
		return nil, fmt.Errorf("failed to parse endpoint: %w", err)
	}
	u.Path = "/authentication_token"

	url := u.String()
	reqBody := &schema.AuthenticationTokenRequest{
		Id:       id,
		Password: password,
	}
	...
	req, err := http.NewRequest("POST", url, bytes.NewReader(reqBodyData))
	if err != nil {
		return nil, err
	}
	...
	var respBody schema.AuthenticationTokenResponse
...
func (c *Client) Do(r *http.Request, v interface{}) (*Response, error) {
	...
	for {
		...
		if err = response.readMeta(body); err != nil {
			return response, fmt.Errorf("rewardcloud: error reading response meta data: %s", err)
		}

		if resp.StatusCode >= 400 && resp.StatusCode <= 599 {
			err = errorFromResponse(resp, body)
			if err == nil {
				err = fmt.Errorf("rewardcloud: server responded with status code %d", resp.StatusCode)
...
func (r *Response) readMeta(body []byte) error {
	...
	// 	strings.HasPrefix(r.Header.Get("Content-Type"), "application/ld+json") {
...
type ListOpts struct {
	Page         int `json:"page"`         // Page (starting at 1)
	ItemsPerPage int `json:"itemsPerPage"` // Items per page (0 means default)
}

func (l ListOpts) values() urlpkg.Values {
	vals := urlpkg.Values{}
	if l.Page > 0 {
		vals.Add("page", strconv.Itoa(l.Page))
	}
	if l.ItemsPerPage > 0 {
		vals.Add("per_page", strconv.Itoa(l.ItemsPerPage))
	}
	return vals
}
